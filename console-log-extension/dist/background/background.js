...
async function handleMessage(message, sender) {
  try {
    ...
    switch (type) {
      ...
      case 'KEEP_ONLY_SESSION':
        return await errorHandler.wrapAsync(
          () => keepOnlySession(data?.sessionId),
          { type: 'keep_only_session', context: 'message_handler' },
...
/**
 * Delete all logs except those belonging to a specific sessionId
 * @param {string} sessionId - The session to keep
 * @returns {Promise<{success:boolean, deletedCount:number}>}
 */
async function keepOnlySession(sessionId) {
  if (!sessionId || typeof sessionId !== 'string') {
    throw new Error('Valid sessionId is required');
  }
  try {
    // Get all logs (bounded by large limit) and filter out the session to keep
    const all = await storageManager.queryLogs({ startTime: 0, endTime: Date.now(), limit: 1000000 });
    const toDelete = (all || []).filter(l => l.sessionId !== sessionId).map(l => l.id);
    const deletedCount = toDelete.length > 0 ? await storageManager.deleteLogs(toDelete) : 0;
    return { success: true, deletedCount };
  } catch (e) {
    throw new Error(`Failed to keep only session: ${e.message}`);
  }
}
...
async function processLogMessage(logData, sender) {
  try {
    ...
    // Determine source: default to inbound data source, otherwise infer from URL
    const inferredSource = (logData && logData.source) ? logData.source : (
      (typeof url === 'string' && url.startsWith('chrome-extension://')) ? 'extension' : 'page'
    );
...
async function shouldCaptureLog(logEntry) {
  // Drop known noisy vendor errors that are not actionable
  try {
    const msg = String(logEntry.message || '');
    if (/Cannot\s+read\s+(properties|property)\s+of\s+null/i.test(msg) && /\bsendMessage\b/i.test(msg)) {
      return false;
    }
  } catch (_) {}
...
async function getLogs(options = {}) {
  try {
    let logs = await storageManager.queryLogs(options);
    // Optionally hide extension logs server-side to reduce payload/noise
    const hideExt = (typeof options.hideExtensionLogs === 'boolean') ? options.hideExtensionLogs : !!(extensionSettings && extensionSettings.hideExtensionLogs);
    if (hideExt && Array.isArray(logs)) {
      logs = logs.filter(l => !isExtensionGeneratedLog(l));
    }
...
/**
 * Determine if a log entry was generated by the extension itself
 * @param {Object} log
 * @returns {boolean}
 */
function isExtensionGeneratedLog(log) {
  if (!log) return false;
  if (log.source === 'extension' || log.isExtension) return true;
  // Heuristics for older data
  if (typeof log.url === 'string' && log.url.startsWith('chrome-extension://')) return true;
  if (typeof log.domain === 'string' && (log.domain === 'chrome-extension' || log.domain.includes('extension'))) return true;
  return false;
}
...
async function getRecentLogs(options = {}) {
  try {
    ...
    // Respect default hide-extension-logs setting when showing quick recent activity
    const hideExt = !!(extensionSettings && extensionSettings.hideExtensionLogs);
    const sortedLogs = ((logs || [])
      .filter(l => !hideExt || !isExtensionGeneratedLog(l))
...
async function getStatistics() {
  try {
    ...
    let todayLogs = await storageManager.queryLogs({
      startTime: todayStartTimestamp,
      endTime: now,
      limit: 10000 // Large limit to get all today's logs
    });
    // Respect hide-extension-logs default for the popup summary to align with options view
    const hideExt = !!(extensionSettings && extensionSettings.hideExtensionLogs);
    if (hideExt && Array.isArray(todayLogs)) {
      todayLogs = todayLogs.filter(l => !isExtensionGeneratedLog(l));
    }
